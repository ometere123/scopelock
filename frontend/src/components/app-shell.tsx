...
export function AppShell({children}:{children:React.ReactNode}){
 ...
 const connect=async()=>{
  setConnectError("");
  try {
   if(wallet.address) await wallet.refresh();
   else await wallet.connect();
  } catch(error) {
   setConnectError(error instanceof Error?error.message:"Wallet connection failed. Try again after unlocking your wallet.");
  }
 };
 ...
 return <><header className="masthead"><Link className="wordmark" href="/"><svg aria-hidden="true" viewBox="0 0 64 64"><path d="M32 7 52 15v15c0 13-8.2 22.8-20 27C20.2 52.8 12 43 12 30V15L32 7Z"/><path className="wordmark-check" d="M24 31.5 29.2 37 41 24.5"/></svg><span>SCOPELOCK</span></Link><nav aria-label="Primary navigation">{links.map(([name,href])=><Link key={href} className={path===href||path.startsWith(`${href}/`)&&href!=="/"?"active":""} href={href}>{name}</Link>)}</nav><button className="connect" disabled={wallet.status==="connecting"} aria-label={wallet.address?"Refresh wallet connection":"Connect wallet"} onClick={()=>void connect()}>{wallet.status==="connecting"?"CONNECTING…":short}</button></header><div className="network-strip">{network}</div>{wallet.error||connectError?<p className="wallet-error" role="alert" aria-live="polite">{connectError||wallet.error}</p>:null}{children}</>;
}
